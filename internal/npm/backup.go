...
const DefaultBackupDirName = ".nvnp/back"
...
func Backup(request BackupRequest) (*BackupReport, error) {
	...
	for _, pkgPath := range packageFiles {
		...
		if err := copyFile(absPath, destPath); err != nil {
			return nil, fmt.Errorf("backup %s: %w", absPath, err)
		}
...
func FormatBackupReport(report *BackupReport) string {
	...
	b.WriteString(fmt.Sprintf("%d package.json file(s) backed up\n", len(report.Manifest.Files)))
	return b.String()
}
...
func FormatRestoreReport(report *RestoreReport) string {
	...
	b.WriteString(fmt.Sprintf("%d package.json file(s) restored\n", report.Files))
	return b.String()
}
